...
class User(db.Model):
    ...
    def __repr__(self): # Fixed _repr_ to __repr__
...
def view_applications(job_id):
    ...
    for app_obj in applications:
        app_data = {
            ...
            'user_name': app_obj.applicant.username,
            'user_email': app_obj.applicant.email
        }
...
def apply_job(job_id):
    ...
    unique_filename = f"{user_id}_{job_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}_{filename}" # Added _ between user_id and job_id
    resume_path = os.path.join(app.config['UPLOAD_FOLDER_RESUMES'], unique_filename)
    resume_file.save(resume_path)
    print(f"DEBUG: Saved resume to: {resume_path}")
    print(f"DEBUG: UPLOAD_FOLDER_RESUMES config: {app.config['UPLOAD_FOLDER_RESUMES']}")
...
def my_applications():
    ...
    for app_obj in user_applications:
        app_data = {
            ...
            'company_name': app_obj.job.hr.username + ' Inc.',
            'applied_at': app_obj.applied_at,
            'match_score': app_obj.match_score,
            'status': app_obj.status
        }
...
def download_resume(filename):
    print(f"DEBUG: Attempting to download filename: {filename}")
    print(f"DEBUG: From directory: {app.config['UPLOAD_FOLDER_RESUMES']}")
    
    full_expected_path = os.path.join(app.config['UPLOAD_FOLDER_RESUMES'], filename)
    print(f"DEBUG: Full expected file path: {full_expected_path}")
    if not os.path.exists(full_expected_path):
        print(f"ERROR: File does NOT exist at: {full_expected_path}")
        # Consider returning an error response here instead of letting send_from_directory raise a 404
        # For example: abort(404, description="File not found or access denied")
    return send_from_directory(app.config['UPLOAD_FOLDER_RESUMES'], filename)

@app.route('/uploads/jds/<path:filename>')
def download_jd(filename):
    print(f"DEBUG: Attempting to download JD filename: {filename}")
    print(f"DEBUG: From directory: {app.config['UPLOAD_FOLDER_JDS']}")
    full_expected_path = os.path.join(app.config['UPLOAD_FOLDER_JDS'], filename)
    print(f"DEBUG: Full expected JD file path: {full_expected_path}")
    if not os.path.exists(full_expected_path):
        print(f"ERROR: JD File does NOT exist at: {full_expected_path}")
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not User.query.first():
            ...
            print("Database already contains users. Skipping initial user creation.")
